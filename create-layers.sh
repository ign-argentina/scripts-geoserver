...
do
    ...
    then
        ...
        xmlDatastore='<dataStore><name>'"$datastore"'</name><type>PostGIS</type><enabled>true</enabled><workspace><name>'"$workspace"'</name><atom:link xmlns:atom="http://www.w3.org/2005/Atom" rel="alternate" href="'"$protocol""$host"'/geoserver/rest/workspaces/'"$workspace"'.xml" type="application/xml"/></workspace><connectionParameters><entry key="port">'"$dbPort"'</entry><entry key="user">'"$dbUser"'</entry><entry key="passwd">'"$dbPassword"'</entry><entry key="dbtype">postgis</entry><entry key="host">'"$dbHost"'</entry><entry key="database">'"$database"'</entry><entry key="schema">'"$schema"'</entry></connectionParameters><__default>false</__default></dataStore>'
